...
<script>
  ...
  import {
    filterTime, judgeLimit, validateDay, clearHistorySearch, isArray, setSearchValue,
    validateMobilePhone
  } from '../../untils/common'
  ...
  export default {
    data: function () {
      return {
        ...
        column: null,
        caseRecordMenuId: 91, // 催记
        CaseDetailMenuId: 92, // 根据用户id跳转到催收详情
        // v_0.7.3
        userId: '',
    ...
    computed: {
      ...mapState(['fixedField', 'caseDataList', 'CaseRecordDialogTableVisible']),
    ...
    methods: {
      ...mapActions(['getCaseDataList', 'buttonLimit']),
      // 设置placeholder
      selectConditions() {
        ...
        if (val === 1) {
          ...
          if (this.flowTimeRange) {
            if (isArray(this.flowTimeRange)) {
              ...
              this.flowTimeRange = [this.flowTimeRange, this.flowTimeRange]
              this.postParam.minNextCallTime = filterTime(this.flowTimeRange[0], 'dateTime')
              this.postParam.maxNextCallTime = filterTime(this.flowTimeRange[1], 'dateTime')
            }
          } else {
      ...
      // 催收案件详情
      goToCaseDetail(row) {
        if (!judgeLimit(this.$store.state.setButtonLimitData, this.CaseDetailMenuId)) return
