  ...
  <el-card style="width:100%;" class="big_background">
    ...
    <el-card>
      <div class="rel">
        ...
        <el-table v-if="setLetterOnData"
                  :data="setLetterOnData.records"
          ...
          <el-table-column label="操作" width="250px">
            <template slot-scope="scope">
              <el-button @click="aduit(scope.row)" type="text"
                         v-show="((parseInt(activeName) === 101 ||
                       parseInt(activeName) === 103) || (scope.row.showAudit && parseInt(activeName) === 2)) && showApproval">
                审批
              </el-button>
              <!---->
              ...
              <el-button @click="openLetterRecord(scope.row)"
                         type="text" v-show="showApprovalRecord">审批记录
              </el-button>
              <el-button @click="openRemarkRecord(scope.row)"
                         type="text" v-show="showRemarkRecord">备注记录
              </el-button>
              <el-button @click="lookEngineAgain(scope.row)"
                         type="text"
                         v-show="showEngineAgain && Number.parseInt(activeName) === 100">重过引擎
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        <div class="tr pt30">
          <el-pagination v-if="setLetterOnData"
                         ...
                         :total="setLetterOnData.totalrecord">
          </el-pagination>
        </div>
      </div>
    </el-card>
    ...
    <mandatory-dialog
      ...
      @refresh="refresh"></mandatory-dialog>
  ...
  export default {
    data: function () {
      return {
        ...
        activeName: '101',
        OrderMenuId: 83, // 接单
        approvalMenuId: 84, // 审批
        ApprovalRecordMenuId: 85, // 审批记录
        RemarkRecordMenuId: 86, // 备注记录
        EngineAgainMenuId: 87, // 重过引擎
        applyDateRange: '',
        approvalDateRange: '',
        // v_0.7.3
        userId: '',
    ...
    computed: {
      ...mapState(['fixedField', 'setLetterOnData', 'auditorListData', 'setButtonLimitData']),
      showOrder: function () {
      ...
      showEngineAgain: function () {
        return judgeLimit(this.$store.state.setButtonLimitData, this.EngineAgainMenuId)
      },
      showDetail: function () {
    ...
    mounted() {
      // 获取审批列表
      this.getLetterOnList(this.postParam)
      // 获取审批人列表
      this.getAuditorList()
      // this.forcedOut()
    },
    ...
    methods: {
      ...mapActions(['getLetterOnList', 'getAuditorList', 'buttonLimit']),
      // 子组件监听父组件的事件
      hideUserDetail() {
            ...
            if (parseInt(res.data.body.status) === 1) {
              this.$router.push({
                ...
                query: {
                  ...
                  showBtn: true,
                  pageType: 2, // 审批订单池和我的审批页面的跳转判断 （2: 审批订单池， 1：我的审批）
                  hrefType: 2 // 审批完成之后，返回审批列表页面|审批详情页的判断 (2: 审批详情 1：审批订单池）
                }
              })
      ...
      // 表格中的审批按钮
      aduit(row) {
        axios.post('auditOrder/orderAuditLocking', {orderId: row.orderId, type: 2}).then(res => {
          if (res.data.code === 200) {
            if (parseInt(res.data.body.status) === 0) {
              ...
              this.$router.push({
                ...
                query: {
                  ...
                  showBtn: true,
                  pageType: 2,
                  hrefType: 1 // 审批后页面的跳转类型判断 (2: 审批详情 1：审批列表)
                }
              })
      ...
      lookEngineAgain(row) {
        axios.post('audit/reRunStrategyEngine', {orderId: row.orderId}).then(res => {
          if (res.data.code === 200) {
